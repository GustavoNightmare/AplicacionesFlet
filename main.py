...
async def main(page: ft.Page):
    ...
    # -----------------------------------------------------------------
    page.title = "Background Remover Pro"       # El título que se muestra en la barra superior.
    page.bgcolor = "#1a1a2e"                   # Color de fondo oscuro y moderno (código hexadecimal).
    ...
    page.window.width = 800                    # Ancho de la ventana en píxeles.
    
    page.theme_mode = ft.ThemeMode.DARK        # Forzamos a que la interfaz use el tema oscuro.
    ...
    async def btn_pick_files_click(e):
        ...
        files = await file_picker.pick_files(
            allow_multiple=True,  # Permite seleccionar varias imágenes a la vez.
            allowed_extensions=["png", "jpg", "jpeg", "bmp", "webp"]  # Solo permite estos formatos.
        )

        # PROCESAMOS EL RESULTADO:
        # Si la lista de archivos no está vacía (el usuario no canceló la selección):
        if files:
            file_count = len(files)                       # Contamos cuántos archivos seleccionó.
            first_file_path = files[0].path               # Obtenemos la ruta completa de la primera imagen.
            directory = os.path.dirname(first_file_path)  # Extraemos la carpeta contenedora.
            ...
            select_files_info.color = "#acaf50"
        else:
    ...
    btn_pick_files = ft.Button(
        ...
        content=ft.Row([
            ft.Icon(ft.Icons.CLOUD_UPLOAD, color="#ffffff"),  # Ícono de una nube con flecha arriba.
            ft.Text("Seleccionar Imágenes", color="#ffffff", weight=ft.FontWeight.BOLD)  # Texto en negrita.
        ], alignment=ft.MainAxisAlignment.CENTER),  # Centramos el contenido dentro del botón.
        ...
        style=ft.ButtonStyle(
            shape=ft.RoundedRectangleBorder(radius=12),  # Bordes redondeados modernos.
